...
function openTome(){
  ...
  const petHtml=s.pet?'<h4>🐾 灵兽</h4><p>'+(typeof petArt==='function'&&petArt(s.pet)?'<img class="pet-mini" src="'+petArt(s.pet)+'" alt="" loading="lazy" onerror="this.style.display=\'none\'">':'')+esc(s.pet.species+'「'+s.pet.name+'」')+' · '+s.pet.level+'级 · '+((typeof petFormStage==='function'&&petFormStage(s.pet))?petFormStage(s.pet).n+'（'+s.pet.form+'阶）':s.pet.form+'阶')+' · '+PET_TALENT_DESC[s.pet.talent]+(typeof petBranchDesc==='function'&&petBranchDesc(s.pet)?' · 分支：'+petBranchDesc(s.pet):'')+'</p>':'';
