...
function battle(enemy,onEnd,spar){
  return new Promise(resolve=>{
    ...
    const qteRound=()=>{
      ...
      const finishQte=(mult,ok)=>{
        if(ok){
          ...
          eh=Math.max(0,eh-dmg);st.dmgDealt+=dmg;st.crits++;bfigHit('enemy');fxSlash('enemy','#ffd76a');
    ...
    const step=()=>{
      ...
      if(burn>0&&eh>0){let bd=Math.max(1,Math.floor(enemy.hp*0.06));if(ot)bd=Math.max(1,Math.floor(bd*1.2));eh=Math.max(0,eh-bd);st.dmgDealt+=bd;bfigHit('enemy');fxSlash('enemy','#e87a3a');html.push('<p class="bl">🔥 灼烧蔓延，<b>'+esc(enemy.name)+'</b> 受创 <span class="bhit">-'+bd+'</span>。</p>')}
      ...
      if(pa>=enemy.def+8){
        ...
        fxSlash('enemy',beatHit?'#e8c86a':(crit?'#ffd76a':'#d8e0f0'));
        if(beatHit)fxFloatText('⚡ 克敌','#e8c86a',false);
      ...
      if(frozen){
        ...
        if(ea>=8+armorDef(S)+dodgeBonus(S)+techAgi){
          ...
          ph-=d;st.dmgTaken+=d;bfigHit('player');fxSlash('player','#e08a8a');
